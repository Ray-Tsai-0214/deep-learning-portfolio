...
from peft import PeftModel  # 添加PEFT庫引用
...
def generate_predictions_batch(model, tokenizer, test_file, api_key, batch_size=8):
    ...
    checkpoint_file = "prediction_checkpoint_finetune_deepseek.csv"
    final_output_file = "submission_api_enhanced_finetuned.csv"  # 更改文件名以反映使用微調模型
...
def main():
    ...
    print("加載tokenizer...")
    tokenizer = AutoTokenizer.from_pretrained(model_path)  # 從微調模型路徑加載tokenizer
